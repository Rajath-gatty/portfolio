...
const UI = ((ProjectCtrl) => {
        ...
        projects.forEach((proj) => {
            html += `
                ...
                <div class="flex-1 relative">
                    <img class="bg-cover project-img w-full blur-img" src="${
                        proj.images.thumbLazy
                    }" data-img="${proj.images.thumb}" data-id="${
                proj.id
            }" alt="${proj.title} Project">
                    <div class="img-thumb-backdrop flex items-center justify-center">
                    <button class="py-[0.75rem] font-bold px-4 text-sm text-white border border-white z-30 more-details-btn" data-id="${
                        proj.id
                    }">More Details</button>
                    </div>
                </div>
                ...
                <div class="p-4 flex-1 space-y-6">
                    <h1 class="lg:text-3xl sm:text-2xl xs:text-2xl font-display-serif">${
                        proj.title
                    }</h1>
                    <p class="lg:text-lg leading-8 xs:text-sm font-open-sans text-gray-light">${
                        proj.description
                    }</p>
                    <div class="flex items-center justify-between">
                        ...
                        <a href="${
                            proj.demoUrl
                        }" class="bg-primary no-underline text-white  lg:font-bold border-none px-4 py-2 lg:text-md xs:text-sm">Live Demo</a>
                        <a href="${
                            proj.githubUrl || "#"
                        }"class="bg-gray-dark no-underline text-white  lg:font-bold border-none px-4 py-2 lg:text-md xs:text-sm">Github</a>
                        </div>
                        <button class="border more-details-btn border-gray-dark no-underline text-gray-dark  lg:font-bold px-4 py-2 lg:text-md xs:text-sm" data-id="${
                            proj.id
                        }">More details</button>
                    </div>
                </div>
    ...
    const populateModal = (id) => {
        ...
        githubUrl.classList.contains("hidden") &&
            githubUrl.classList.remove("hidden");
        ...
        project.features.forEach(
            (fea) =>
                (listsHtml += `<li class="text-md  text-gray-light pl-3">${fea}</li>`)
        );
        ...
        project.images.altImages.forEach((img, i) => {
            imagesHtml += `<img class="w-28 border cursor-pointer alt-image border-gray-light/30 p-1 rounded-md" src="${
                img.imgLazy
            }" data-src="${img.img}" data-id="${project.id}" data-pos="${
                i + 1
            }" alt="blog"/>`;
        });
        ...
        let techUsed = "";
        project.techUsed.forEach(
            (tech) =>
                (techUsed += `<span class="px-3 py-2 border text-sm text-gray-light rounded-md border-primary">${tech}</span>`)
        );
...
const App = ((ProjectCtrl, UI) => {
    const loadEventListeners = () => {
        ...
        document
            ...
            .forEach((el) => el.addEventListener("click", UI.closeModal));
        document
            .querySelector(".modal-alt-img")
            .addEventListener("click", UI.changeAltImages);
    };
